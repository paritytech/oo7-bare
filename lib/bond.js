...
//			console.log(`firing (${JSON.stringify(v)})`);
	 ...
	 */
	map (transform, outResolveDepth = 3, cache = undefined) {
	 ...
	 * Transformation.
	 */
	mapEach(transform, extraOutResolveDepth = 3, cache = undefined) {
		return this.map(item => item.map(transform), 1 + extraOutResolveDepth, cache);
	}
	 ...
	 * `name`.
	 */
	sub (name, outResolveDepth = 3, cache = undefined) {
		const TransformBond = require('./transformBond');
	 ...
	 * that a {@link Bond} may be for it to be resolved.
	 */
	static mapAll(list, transform, outResolveDepth = 3, resolveDepth = 3, cache = undefined) {
		const TransformBond = require('./transformBond');
